...
class TCN_ResidualBlock2(nn.Module):
    def __init__(
        self,
        num_filters: int,
        kernel_size: int,
        ...
        if input_dim != output_dim:
            self.conv3 = nn.Conv1d(input_dim, output_dim, 1).cuda()
...
class PatchMixer(nn.Module):
    def __init__(self, configs):
        ...
        if self.num_layers is None and self.dilation_base > 1:
            ...
            num_layers = math.ceil(
                (self.patch_num - 1) / (self.kernel_size - 1) / 2
            )
        self.num_layers = num_layers
    ...
    def forward(self, x): # B, L, D -> B, H, D
        ...
        # print("BEFORE TCN:",z.shape)
        for res_block in self.res_blocks_list:
            z = res_block(z)
        # print("AFTER TCN:",z.shape)
        z=z.reshape(self.batch_size,self.enc_in,-1)

        # 4
        z_mlp = self.mlp(z) # B, D, L * P -> B, D, H
